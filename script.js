...
const closeFormModalBtn = document.getElementsByClassName("modalFormSuccessBtn");
...
const modalMessages = {
    contactForm: {
        ...
        text_1: "We have received it correctly.",
        text_2: "Someone from our team will get back to you very soon.",
    },
    NewsletterSubscribe: {
        title: "¡Thank you for your interest!",
        text_1: "We have received the subscription correctly.",
        text_2: "You will receive all the news in your email account.",
    },
};
...
function makeModalContent(option) {
    modalTitle.innerText = modalMessages[option].title;
    modalText1.innerText = modalMessages[option].text_1;
    modalText2.innerText = modalMessages[option].text_2;
}

function modalFormBehaviour() {
...
function modalBehavior() {
    ...
    formNews.addEventListener("submit", async (e) => {
        ...
        if (!valid) {
            ...
            const jsonData = await response.json();
            console.log(jsonData.body);
            makeModalContent("NewsletterSubscribe");
            formModal.style.display = "block";
            modalFormBehaviour();
...
function setPrices(option) {
    ...
    function writePrices(factor, unit) {
        for (let index = 0; index < prices.length; index++) {
            prices[index].innerHTML = `${Math.round(defaultPrices[index] * factor)} <small class="priceUnit">${unit}</small`;
        }
    }
    switch (option) {
        case 0:
...
class Slider {
    ...
    getDotsList() {
        ...
        const positionMarkItems = [...positionMarksDiv.getElementsByClassName("slider__position-item")];
    ...
    autoChange(interval) {
        setInterval(() => {
            new Date() - this.lastMove >= interval && this.handleControlsClick("next");
        }, interval);
    }
}
